    ...
    return (
      <div styleName='slider-container'>
        <div styleName='slider' onClick={this.onClick}>
          <div styleName='slider-bar' ref={(node) => { this.domNode = node; }}>
            {max > 0
              ? (
            ...
            {this.props.shoutOuts.map(
              (shoutOut, index) => {
                return shoutOut === this.props.currentShoutOut ? <BigIcon max={max} timeSpot={shoutOut.timespot} key={index}/> 
                : <SmallIcon timeSpot={shoutOut.timespot} max={max} key={index}/> 
              }
            )}           
          </div>
        </div>
